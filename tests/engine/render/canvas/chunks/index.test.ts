...
describe('render chunk spatial index', () => {
  ...
  test('keeps generated-effect subtrees atomic so the effect renders after descendants', () => {
    const { graph, pageId } = graphWithNodes(64)
    const frame = graph.getChildren(pageId)[0]
    if (!frame) throw new Error('Expected frame')
    graph.updateNode(frame.id, { generatedEffect: generatedEffect('noise') })

    const { index, stats } = RenderChunkIndex.build(graph, pageId)
    const chunks = index.getChunksForNode(frame.id)

    expect(chunks).toHaveLength(1)
    expect(chunks[0]?.kind).toBe('subtree')
    expect(chunks[0]?.nodeCount).toBe(65)
