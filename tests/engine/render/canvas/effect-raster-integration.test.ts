...
function renderPixels(renderer: SkiaRenderer, graph: SceneGraph): Uint8Array {
  ...
  do {
    ...
    buildSteps++
    if (buildSteps > 128) throw new Error('Retained scene backing did not settle')
  } while (renderer.sceneBackingBuild)
...
describe('retained effect raster integration', () => {
  ...
  test('nested shadow subtrees stay out of the top-level effect raster path', () => {
    const graph = new SceneGraph()
    const page = graph.getPages()[0]
    if (!page) throw new Error('Expected default page')
