...
test('canonical authoring reference retains lowcode controls and structural round-trip boundaries', () => {
  expect(JSX_REFERENCE).toContain('## Lowcode Control Props')
  expect(JSX_REFERENCE).toContain('interactiveProps={{...}}')
  expect(JSX_REFERENCE).toContain('update_lowcode_node')
  expect(JSX_REFERENCE).toContain(
    'but not `bindings`, `events`, `stateOverrides`, or `renderCondition`'
  )
})
