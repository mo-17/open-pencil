/* oxlint-disable eslint(complexity), eslint(max-lines) -- The stateful fetch doubles model and native error matrix cover one bounded Supabase staging project end to end. */
...
import { DesktopSupabaseBackendStagingVerificationError } from '@/app/plugins/host/deployment/desktop/supabase/backend/staging/verification'
import {
    ...
    await withProductionCredentials(async () => {
      for (const [nativeCode, expectedCode] of cases) {
        ...
        await mockTauriIPC((command, args) => {
          ...
          throw { code: nativeCode, message: `provider-controlled ${READ_PAT}` }
        })
        ...
        try {
          ...
          let error: unknown
          try {
            await service.verify(verificationInput(prepared))
          } catch (cause) {
            error = cause
          }
          expect(error).toBeInstanceOf(DesktopSupabaseBackendStagingVerificationError)
          expect((error as DesktopSupabaseBackendStagingVerificationError).code).toBe(expectedCode)
          expect(JSON.stringify(error)).not.toContain(READ_PAT)
          expect(calls).toBe(1)
        } finally {
