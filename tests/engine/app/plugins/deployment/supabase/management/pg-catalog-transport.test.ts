...
import type { QueryBody } from '#tests/engine/app/plugins/deployment/supabase/management/helpers'
...
type SupabaseReadOnlyQueryBody = QueryBody
...
describe('Supabase Management fixed pg_catalog transport', () => {
  test('uses GET authority plus exactly one fixed read-only aggregate POST', async () => {
    ...
    expect(body.query).toContain("'pg_catalog.pg_class'::pg_catalog.regclass")
    expect(body.query).not.toMatch(/::(?:text|integer|boolean|jsonb|regclass|oid|"char")\b/u)
