...
test('app plugin IDB storage increments one shared atomic revision per mutation', async () => {
  ...
  const first = createIdbAppPluginStateStorage(databaseName, fakeIndexedDB)
  const second = createIdbAppPluginStateStorage(databaseName, fakeIndexedDB)
  ...
  expect((await first.list()).map((value) => Reflect.get(value, 'pluginId'))).toEqual(['plugin.b'])
})
