...
describe('stripeCheckout action (Phase 5 §12)', () => {
  ...
  test('emits POST to author endpoint and redirects to returned checkout URL', () => {
    ...
    expect(out).toContain('const nextUrl = new URL(checkoutUrl, window.location.href)')
    expect(out).toContain('if (nextUrl.protocol !== "https:" && nextUrl.protocol !== "http:")')
    expect(out).toContain('throw new Error("stripeCheckout response url must be http(s)")')
