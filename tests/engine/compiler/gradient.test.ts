...
describe('compile — gradient fills (Phase 4 §24.2)', () => {
  ...
  test('angular gradient compiles to a conic-gradient arbitrary-value class', () => {
    const graph = makeSceneGraph()
    const pageId = firstPageId(graph)
    graph.createNode('RECTANGLE', pageId, {
      name: 'Conic',
    ...
    expect(app).toContain('bg-[conic-gradient(from_90deg_at_50%_50%,_#FF0000_0%,_#0000FF_100%)]')
