...
test.describe('split canvas views', () => {
  test('splits, resizes, activates, and closes independent canvas views', async ({ page }) => {
    ...
    await expect(page.getByTestId('motion-path-overlay')).toHaveCount(1)
    await expect(panes.nth(0).getByTestId('motion-path-overlay')).toHaveCount(0)
    await expect(panes.nth(1).getByTestId('motion-path-overlay')).toHaveCount(1)
    ...
    await expect(panes.nth(0).getByTestId('motion-path-overlay')).toHaveCount(1)
    await expect(panes.nth(1).getByTestId('motion-path-overlay')).toHaveCount(0)
