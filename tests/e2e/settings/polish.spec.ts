...
for (const reducedMotion of ['no-preference', 'reduce'] as const) {
  test(`Settings disclosures open, close and reopen with ${reducedMotion} motion`, async ({
    ...
    const trigger = page.getByRole('button', { name: 'Available tools', exact: true })
    ...
    await expect(page.getByRole('searchbox', { name: 'Search MCP tools' })).toBeVisible()
    await trigger.click()
    await expect(page.getByRole('searchbox', { name: 'Search MCP tools' })).toBeHidden()
