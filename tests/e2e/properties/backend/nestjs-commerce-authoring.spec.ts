...
test('authors NestJS public access and declared queries, then binds LIST and request controls', async ({
  ...
  await expect(security.getByRole('button', { name: '+ Tenant', exact: true })).toBeVisible()
  await expect(security).toContainText('stores.owner_id')
  await expect(security).toContainText('complex membership rules must pass provider validation')
