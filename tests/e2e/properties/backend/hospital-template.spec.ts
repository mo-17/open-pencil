...
test('creates eleven hospital registration pages with role review and atomic undo and redo', async ({
  page
}, info) => {
  ...
  const title = 'Hospital registration'
  await library.getByRole('button', { name: title, exact: true }).click()
  const details = library.getByRole('region', { name: title, exact: true })
  await expect(details).toContainText('hospital-admin')
  await expect(details).toContainText('hospital-staff')
  ...
  const titles = [
    ...
    'Manage departments',
    'Manage doctors',
    'Manage hospital sessions',
    'Registration desk'
  ]
  for (const name of titles) await expect(details).toContainText(name)
  await page.screenshot({ path: info.outputPath('hospital-template-requirements.png') })
  ...
  for (const name of titles) await expect(pages.getByText(name, { exact: true })).toBeVisible()
  await settle()
  await page.screenshot({ path: info.outputPath('hospital-template-created.png') })
  ...
  for (const name of titles) await expect(pages.getByText(name, { exact: true })).toBeVisible()
  await settle()
  await page.screenshot({ path: info.outputPath('hospital-template-restored.png') })
