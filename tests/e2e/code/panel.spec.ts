...
test('selecting a button shows JSX code', async () => {
  ...
  await openCodePanel()
  await expect(codeEditor()).toContainText('Button')
  await expect(codeEditor()).toContainText('Continue')
...
test('selecting an input shows placeholder code', async () => {
  ...
  await editor.canvas.waitForRender()
  await openCodePanel()
  await expect(codeEditor()).toContainText('Input')
  await expect(codeEditor()).toContainText('Email address')
  await expect(codeEditor()).toContainText('hello@example.com')
})
...
test('selecting a select shows option code', async () => {
  ...
  await editor.canvas.waitForRender()
  await openCodePanel()
  await expect(codeEditor()).toContainText('Select')
  await expect(codeEditor()).toContainText('US')
  await expect(codeEditor()).toContainText('JP')
})
