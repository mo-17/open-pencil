...
export async function verifyArtifactConsumers(root: string, tarballs: string[]): Promise<void> {
  ...
  const publishedScope = inspections.every(({ manifest }) =>
    manifest.name.startsWith('@open-pencil-lowcode/')
  )
  const scenarios = publishedScope
    ? runtimeScenarios.map((scenario) => ({
        ...scenario,
        code: scenario.code.replaceAll('@open-pencil/', '@open-pencil-lowcode/')
      }))
    : runtimeScenarios
