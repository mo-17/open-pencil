...
test.each(['p2-motion-e2e', 'desktop-tests'])(
  'requires the fork %s suite for code changes and skips it for docs',
  (job) => {
    for (const result of ['failure', 'cancelled', 'skipped'] as const) {
      expect(gateErrors({ ...results('code'), [job]: { result } })).toContain(
        `${job} did not succeed`
      )
    }
    const incomplete = Object.fromEntries(
      Object.entries(results('code')).filter(([name]) => name !== job)
    )
    expect(gateErrors(incomplete)).toContain(`${job} did not succeed`)
    expect(gateErrors({ ...results('docs'), [job]: { result: 'success' } })).toContain(
      `${job} was not skipped for docs-only routing`
    )
  }
)
