  ...
  (sourceRel, specifier, resolved) => {
    ...
    if (
      ...
      (specifier === '@open-pencil/scene-graph' ||
        ...
        resolved === 'packages/scene-graph/src' ||
        resolved?.startsWith('packages/scene-graph/src/'))
    ) {
      return 'compiler/adapters/** must not import @open-pencil/scene-graph. Adapters depend only on the IR types in compiler/ir/types.ts.'
    }
    return null
  }
)
