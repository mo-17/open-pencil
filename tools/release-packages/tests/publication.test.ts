...
function fixture() {
  ...
  const operations: PublicationOperations = {
    ...
    verifyPublished: async (_root, entry, tarball) => {
...
test('an already-published integrity mismatch prevents every pending registry write', async () => {
  const state = fixture()
  const first = state.plan[0]
  if (!first) throw new Error('Missing fixture entry')
  first.status = 'published'
  state.operations.verifyPublished = async () => {
