...
**锁定范围**:发布库文档里的 COMPONENT / COMPONENT_SET master。生成和返回 manifest 数据,并把 publish metadata 写回 master。消费文档导入、更新传播、manifest 文件 I/O 和 CLI 留后续。
...
- `publishLibraryComponent` 行为:
  - 只接受 COMPONENT / COMPONENT_SET。
  - `componentKey` 优先级:`options.componentKey` → `libraryComponentKey` → `componentKey` → 新生成 40-char hex。
  - 写回 `componentKey`,并同步 `libraryComponentKey/libraryId/libraryVersion/libraryReadonly`。
  - 返回 `{ component, manifest }`;manifest 可由后续 CLI/库文件写入磁盘。
- `componentSubtreeVersion` 使用同步 deterministic structural hash:
  - 保留子节点顺序和可见结构/样式字段。
  - 排除 `id/parentId/source/pluginData/componentKey/libraryVersion/...` 等易漂移 publish 元数据。
  - 输出形如 `v1-<16hex>`。
- 新增 `publish_component` ToolDef:
  - 参数:`component_id`, `library_id`, 可选 `library_name`, `component_key`, `source_kind`, `source_ref`, `readonly`。
  - 返回 `{ ok:true, data:{ component, manifest } }`。
  - 注册到 `EXTENDED_TOOLS`,不进入默认 `CORE_TOOLS`。

**验证**:

- `bun test tests/engine/scene-graph/libraries.test.ts` → 3/0。
- `bun test tests/engine/tools/lowcode/library.test.ts` → 2/0。
- `git diff --check` → exit 0。
- `bun run check:vue` → exit 0。
- `bun run lint:structure` → exit 0(仅既有 max-lines warnings)。
- `bun run test:type-shapes` → 0 duplicate。
...
**锁定范围**:消费文档 cached master 与 library manifest 的版本检测和接受更新。只做 scene-graph helper,不做 manifest 文件 I/O、不做 CLI、不做 GUI、不做 override 智能 remap。

**实现**:

- `checkLibraryUpdates({ targetGraph, manifest })`:
  - 读取 target root `lowcodeLibraries` 中匹配 `manifest.libraryId` 的 imported components。
  - 返回逐 component 的 `up-to-date` / `outdated` / `missing-manifest` / `missing-cached-master` 状态。
  - 对比 source of truth 为 cached master `libraryVersion`(fallback imported ref version) vs manifest entry `version`。
- `acceptLibraryUpdate({ sourceGraph, targetGraph, manifest, componentKey })`:
  - 找到 target cached master,保持 cached master id 不变。
...
- `bun test tests/engine/scene-graph/libraries.test.ts` → 11/0:
  - up-to-date / outdated / missing-manifest / missing-cached-master。
  - accept 后 cached master id 保持不变。
  - 结构相同更新保留 cached child id 并同步 existing instance。
  - 结构变化返回 warning。
  - missing key / missing cached master / missing source error branches。
