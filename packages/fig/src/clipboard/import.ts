...
export interface ClipboardImportOptions {
  /** Owning hosts may restore their serialized node metadata before graph creation. */
  mapNodeProps?: (
    change: KiwiNodeChange,
    converted: ReturnType<typeof nodeChangeToProps>
  ) => ReturnType<typeof nodeChangeToProps>
  /** Runs after instance population, inside source-metadata preservation. */
  afterPopulate?: (created: ReadonlyMap<string, string>) => void
}
...
export function importClipboardNodes(
  ...
  function createNode(figmaId: string, ourParentId: string) {
    ...
    const converted = nodeChangeToProps(nc, blobs)
    const { nodeType, ...props } = options.mapNodeProps?.(nc, converted) ?? converted
