...
import { isFigmaOpaqueNodeType } from './convert'
...
import { variableBindingFieldToKiwi } from './variable-bindings'
import {
...
function serializeVariableBindings(
  ...
  for (const [field, varId] of Object.entries(node.boundVariables)) {
    ...
    // Dynamic gradient-stop fields are represented by Paint.stopsVar instead.
    // They are not members of the Kiwi VariableField enum and must never be
    // emitted through variableConsumptionMap.
...
export function sceneNodeToKiwi(
  ...
  return sceneNodeToKiwiWithContext(node, parentGuid, childIndex, localIdCounter, {
    ...
    getAdditionalPluginData: (pluginNode, pluginGraph) =>
      runtime.getAdditionalPluginData?.(pluginNode, pluginGraph) ?? [],
