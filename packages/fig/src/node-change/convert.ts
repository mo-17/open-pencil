...
function visibleContainerDerivedLayout(
  ...
  // auto-layout children. The import path pins a child to `derived.x/y` when
  // present, so a zeroed (0,0) derived position snaps every such child to the
  // origin. Real Figma files keep genuine child offsets, so only carry x/y
  // when the transform is actually non-zero; otherwise Yoga re-derives it.
