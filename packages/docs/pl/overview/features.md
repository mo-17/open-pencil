...
Naciśnij <kbd>⌘</kbd><kbd>J</kbd>. Ponad 50 wyselekcjonowanych narzędzi tworzy figury, zmienia style i układ, pracuje z komponentami i zmiennymi, wykonuje operacje logiczne, analizuje tokeny projektu i eksportuje zasoby. Można podłączyć Anthropic, OpenAI, Google AI, OpenRouter albo zgodny punkt końcowy.
...
Claude Code, Cursor, Windsurf i inni klienci MCP mogą odczytywać i zmieniać `.fig` bez interfejsu. Serwer oferuje ponad 140 operacji projektowych oraz operacje na dokumentach i plikach przez stdio i HTTP.
