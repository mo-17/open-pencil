...
OpenPencil zawiera [komendę `eval`](/programmable/cli/scripting) oferującą API Plugin kompatybilne z Figmą do skryptowania headless. Ponadto wbudowany chat oferuje ponad 50 wyselekcjonowanych narzędzi, a serwer MCP ponad 140 operacji projektowych przez stdio i HTTP. Penpot ma system pluginów z sandboxem, ale bez API skryptowania headless ani integracji MCP.
...
Open Pencil jest architektonicznie szczuplejszy — jednoprocesowy renderer CanvasKit w ~26K LOC TypeScript, kompatybilny z Figmą z założenia. Penpot to platforma full-stack z ~299K LOC. Open Pencil ma skryptowanie headless, **ponad 50 wbudowanych narzędzi AI**, **ponad 140 operacji MCP**, eksport SVG i natywną aplikację desktop.
