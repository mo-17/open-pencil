...
<kbd>⌘</kbd><kbd>J</kbd> ouvre l’assistant. Son ensemble sélectionné de plus de 50 outils crée des formes, modifie styles et dispositions, travaille avec les composants et variables, exécute des opérations booléennes, analyse les jetons de design et exporte des ressources. Anthropic, OpenAI, Google AI, OpenRouter et les points d’accès compatibles sont pris en charge.
...
Claude Code, Cursor, Windsurf et les autres clients MCP peuvent lire et modifier des fichiers `.fig` sans interface. Le serveur fournit plus de 140 opérations de design ainsi que des opérations sur les documents et fichiers via stdio et HTTP.
