...
La commande [`eval`](/programmable/cli/scripting) fournit une API compatible avec les plugins Figma pour les scripts sans interface, les opérations par lots et les tests automatisés. Le chat intégré offre plus de 50 outils sélectionnés et le serveur MCP plus de 140 opérations de design via stdio et HTTP.
