...
L’assistant intégré exécute plus de 50 outils sélectionnés pour les tâches courantes de l’éditeur. Une instruction peut modifier les ombres de plusieurs boutons, créer un composant avec une variante sombre ou exporter une animation. Les opérations avancées restent disponibles via MCP.

[Chat avec l’AI →](./ai-chat)

## MCP

Claude Code, Cursor, Windsurf et d’autres clients MCP peuvent utiliser plus de 140 opérations de design ainsi que des opérations MCP sur les documents et fichiers. Le chat intégré emploie volontairement un ensemble plus restreint de plus de 50 outils sélectionnés. Le serveur prend en charge stdio et HTTP avec des sessions indépendantes.
