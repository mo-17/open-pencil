...
OpenPencil inclut une [commande `eval`](/programmable/cli/scripting) offrant une API Plugin compatible Figma pour le scripting headless. De plus, le chat intégré offre plus de 50 outils sélectionnés et le serveur MCP plus de 140 opérations de design via stdio et HTTP. Penpot a un système de plugins avec exécution sandboxée mais pas d'API de scripting headless ni d'intégration MCP.
...
Open Pencil est architecturalement plus léger — un renderer CanvasKit monoprocessus en ~26K LOC TypeScript, compatible Figma par conception. Penpot est une plateforme full-stack avec ~299K LOC. Open Pencil a le scripting headless, **plus de 50 outils IA intégrés sélectionnés**, **plus de 140 opérations MCP**, export SVG et une app desktop native.
