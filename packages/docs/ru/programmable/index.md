...
Встроенный ассистент имеет доступ к более чем 50 отобранным инструментам для типичных задач редактора. Опишите, что вам нужно, на естественном языке — «добавь тень 16px ко всем кнопкам», «создай компонент карточки с вариантом для тёмной темы», «экспортируй эту анимацию». Расширенные операции остаются доступны через MCP.
...
Подключите Claude Code, Cursor, Windsurf или любой MCP-совместимый клиент к OpenPencil. Сервер предоставляет более 140 операций с дизайном, а также MCP-операции жизненного цикла документов и файлов. Встроенный чат намеренно использует меньший отобранный набор из более чем 50 инструментов. Работает через stdio или HTTP с поддержкой сессий.
