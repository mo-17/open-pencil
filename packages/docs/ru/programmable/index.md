...
Встроенный помощник использует более 50 отобранных инструментов для типичных задач редактора. Опишите задачу обычным языком, например: «добавь всем кнопкам тень 16 пикселей», «создай компонент карточки с вариантом для тёмной темы» или «экспортируй эту анимацию». Расширенные операции остаются доступны через MCP.
...
Claude Code, Cursor, Windsurf и другие клиенты MCP могут использовать более 140 операций с дизайном, а также MCP-операции с документами и файлами. Встроенный чат намеренно использует меньший отобранный набор из более чем 50 инструментов. Сервер поддерживает stdio и HTTP с сеансами.
