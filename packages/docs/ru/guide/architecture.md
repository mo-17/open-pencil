...
MCP-сервер предлагает более 140 операций с дизайном и операции с документами/файлами. Встроенный чат использует более 50 отобранных инструментов. Включает XPath-запросы (`query_nodes`), инспекцию JSX (`get_jsx`, `diff_jsx`), семантическое описание (`describe`) и визуальную проверку (`export_image` возвращает изображения модели).
...
MCP-сервер предлагает более 140 операций с дизайном и операции с документами/файлами. Встроенный ИИ намеренно использует более 50 отобранных инструментов; CLI предоставляет всю поверхность ToolDef через `eval`.
