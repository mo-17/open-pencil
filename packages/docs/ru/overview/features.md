...
Нажмите <kbd>⌘</kbd><kbd>J</kbd>, чтобы открыть ИИ-ассистента. Более 50 отобранных инструментов создают фигуры, задают стили, управляют макетом, работают с компонентами и переменными, выполняют булевы операции, анализируют дизайн-токены и экспортируют ассеты. Подключайте Anthropic, OpenAI, Google AI, OpenRouter или совместимый эндпоинт.
...
Подключайте Claude Code, Cursor, Windsurf или любой MCP-клиент для чтения и записи файлов `.fig` в headless-режиме. Сервер предлагает более 140 операций с дизайном и операции с документами/файлами через stdio и HTTP.
