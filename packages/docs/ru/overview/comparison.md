...
Команда [`eval`](/programmable/cli/scripting) предоставляет совместимый с Figma Plugin API для сценариев без интерфейса, пакетных операций и автоматизированных тестов. Встроенный чат предлагает более 50 отобранных инструментов, а MCP-сервер — более 140 операций с дизайном через stdio и HTTP.
...
OpenPencil — компактный редактор в одном процессе с CanvasKit и нативной поддержкой `.fig`. Penpot — полная клиент-серверная платформа на Clojure, ClojureScript, Rust, базах данных и службах Docker. Оба поддерживают совместную работу, но используют разные модели. OpenPencil предлагает сценарии без интерфейса, **более 50 встроенных отобранных ИИ-инструментов** и **более 140 MCP-операций**.
