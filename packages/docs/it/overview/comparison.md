...
Il comando [`eval`](/programmable/cli/scripting) fornisce un’API compatibile con i plugin Figma per script senza interfaccia, operazioni in serie e test automatizzati. La chat integrata offre oltre 50 strumenti selezionati e il server MCP oltre 140 operazioni di design tramite stdio e HTTP.
...
OpenPencil è un editor compatto in un solo processo con CanvasKit e supporto nativo `.fig`. Penpot è una piattaforma client-server completa con Clojure, ClojureScript, Rust, database e servizi Docker. Entrambi supportano la collaborazione con modelli diversi. OpenPencil offre scripting senza interfaccia, **oltre 50 strumenti AI integrati selezionati** e **oltre 140 operazioni MCP**.
