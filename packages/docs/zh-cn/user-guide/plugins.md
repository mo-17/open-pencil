...
想自己制作插件，可以直接打开[第三方插件可视化入门教程](/zh-cn/development/plugin-tutorial)。
教程提供四个可运行的 Manifest v2 示例，并把宿主适配、签名、Portal 预检、发布和安装串成
一条完整路径。
...
当前 **Unreleased** 源码线包含 71 个经过审查的插件、76 项 contribution：21 个模块、
13 个命令、13 个导出器、22 个连接器、4 个存储服务商与 3 个 Backend Provider。只有定义、渲染器、合同或导出源码
文件，并不代表插件已经可以使用；该 contribution 还必须完成中央主机注册。新配置中 Map、
Google Drive Storage、OneDrive Storage、Aliyun Drive Storage、Baidu Netdisk Storage、Supabase Backend Provider、NestJS Backend Provider、Compiler Preview Popout 与 AI Popout 默认安装并启用，
其余内置插件都需要用户选择安装并启用。
...
- **Expo React Native Exporter**：把当前文档中受支持的静态设计和原生控件外壳打包为
  真正的 Expo + React Native + TypeScript 源码工程。已配置的 VR 看房模块使用明确标注的
  WebView 组件，其余页面使用原生组件。导出时不安装依赖，也不调用移动端构建工具。Web 专属或尚未原生适配的能力会记录在
  `EXPORT_WARNINGS.md`，不会静默冒充原生实现。
- **Flutter Exporter**：把当前文档中受支持的静态设计打包为 source-only 的 Dart + Flutter
  源码工程，直接生成 Flutter Widget 与导航源码，已配置的 VR 看房模块使用 WebView 组件。导出时不安装依赖、不生成平台
...
地址。Compiler 预览在你点击 **加载视频** 前不会挂载视频或封面地址；生成的 Web/Tauri
...
需要把播放器接到视频目录、创作者表单与私人收藏时，请使用[视频网站与直播频道模板](#视频网站与直播频道模板)。
...
启用 **NestJS Backend Provider**，打开 **后端库**，选择 **个人博客** 或 **汽车资讯**，核对角色和生成页面后点击 **使用模板**。这两个内置业务模板无需额外画布插件。创建属于一次可撤销的文档操作，不会启动 PostgreSQL、配置 OIDC 或授予角色。

个人博客生成 **7 个页面**，其中包括共享登录和账号设置：

| 页面            | 路由                                 |
| --------------- | ------------------------------------ |
| 博客分类        | `/blog/categories`                   |
| 博客阅读        | `/blog`                              |
| 我的博客收藏    | `/blog/bookmarks`                    |
| 博客分类管理    | `/blog/author/categories`            |
| 博客写作        | `/blog/author/articles`              |
| 账号设置 / 登录 | `/account-setup` / `/business-login` |

由身份服务管理员授予 `blog-author`。作者维护共享分类目录，并编写、编辑、发布或撤回**本人文章**。普通登录读者无需作者角色即可管理本人收藏。登记账号资料不会获得发布权限，创建文章也无需选择账号资料。

汽车资讯生成 **12 个页面**，其中包括登录和账号设置：

| 页面            | 路由                                 |
| --------------- | ------------------------------------ |
| 资讯分类        | `/automotive/categories`             |
| 汽车品牌        | `/automotive/brands`                 |
| 车型目录        | `/automotive/models`                 |
| 汽车资讯阅读    | `/automotive/news`                   |
| 我的资讯收藏    | `/automotive/bookmarks`              |
| 资讯分类管理    | `/automotive/editor/categories`      |
| 汽车品牌管理    | `/automotive/editor/brands`          |
| 车型管理        | `/automotive/editor/models`          |
| 汽车资讯编辑    | `/automotive/editor/articles`        |
| 汽车资讯发布    | `/automotive/publisher/articles`     |
| 账号设置 / 登录 | `/account-setup` / `/business-login` |

`auto-editor` 维护分类、品牌和车型，并创建或编辑本人的草稿。`auto-publisher` 核对全部文章并负责发布、撤回，仅有此角色不获得编辑权限；同一人负责两项工作时须同时获得两个角色。车型所属品牌创建后不可更换，文章选定车型后由服务器确定品牌并保存目录名称快照。

可按以下顺序体验：

1. 新增启用的分类。汽车资讯还需新增启用的品牌及其车型。这些是人工维护的资料，不是实时车型配置、在售状态或报价。
2. 创建草稿，选择目录条目，填写标题、摘要和正文。正文最多 8,192 字符，按**纯文本**展示。草稿摘要及正文可留空，发布前标题和正文须非空；HTML 和 Markdown 不会执行或渲染为富文本。
3. 选择草稿进行编辑，仅草稿可编辑。汽车资讯需由具有 `auto-publisher` 的账号进入发布页完成发布。服务器重新检查发布权限及当前分类、车型和品牌是否启用。
4. 在公开阅读页选中文章，正文可在文本区域内滚动。登录读者可收藏文章，再到本人收藏页直接读取当前公开内容。
5. 取消收藏会保留停用的引用记录；文章仍已发布时可恢复原收藏。收藏仅保存文章引用，不复制标题或正文。文章撤回后，新的公开查询不再返回文章；已经下载的内容无法收回。
6. 已发布文章须先撤回为草稿，编辑后再发布。停用分类、车型或品牌会阻止新的发布，但不会自动撤回已有文章。点击 **刷新列表** 获取当前数据，页面不承诺实时更新。

生成的 React 和 Vue 应用覆盖浏览器阅读及编辑表单。评论、富文本编辑、上传、抓取、资讯源、实时车价、定时发布、SSR/SSG 文章路由及 SEO 收录保证均不在当前范围；可在导出后对接相应服务、生产身份与部署配置。原生移动端和小程序发布流程仍需独立实现及设备测试。与其他模块组合时继续遵守现有后端文档预算，超限组合会拒绝，不会静默裁剪。

## 医院挂号模板

后端库中的 **医院挂号** 模板（`hospital-registration`）提供单医院目录、就诊人资料、挂号
预约和窗口行政处理。保持 **NestJS Backend Provider** 启用，打开 **浏览后端库**，核对
模板准备要求后点击 **使用模板**。无需额外画布插件。创建会一次增加 11 个可编辑页面，
支持一步撤销；新文档保留原默认页，因此共有 12 页。

| 页面         | 路由                           | 用途                                     |
| ------------ | ------------------------------ | ---------------------------------------- |
| 业务系统登录 | `/business-login`              | 通过配置的 OIDC 身份服务登录。           |
| 账号设置     | `/account-setup`               | 登记当前账号资料。                       |
| 医院科室     | `/hospital/departments`        | 浏览启用科室及其医生。                   |
| 医生目录     | `/hospital/doctors`            | 搜索医生并查看开放排班。                 |
| 排班挂号     | `/hospital/registration`       | 选择排班及已保存的就诊人，预约一个号源。 |
| 我的就诊人   | `/hospital/patients`           | 新增、编辑或停用本人账号管理的就诊人。   |
| 我的挂号     | `/hospital/appointments`       | 查看、取消或恢复本人预约及操作记录。     |
| 科室管理     | `/hospital/admin/departments`  | 新增、编辑科室目录。                     |
| 医生管理     | `/hospital/admin/doctors`      | 新增、编辑医生目录。                     |
| 医院排班管理 | `/hospital/admin/slots`        | 创建排班、调整号源上限或开放状态。       |
| 挂号处理     | `/hospital/admin/appointments` | 院方取消、签到和完成行政处理。           |

按导出 README 配置 PostgreSQL 与 OIDC。身份管理员为目录及排班管理员授予
`hospital-admin`；`hospital-admin` 和 `hospital-staff` 都可使用挂号处理工作台。
账号登记不会自动授予角色。就诊人主资料仅本人账号可以读写；院方工作台读取预约中的资料
快照，不能借此浏览所有账号的就诊人主资料。

初始化时先登记管理员资料，再创建启用科室及其医生。创建排班时先选科室，再选该科室医生，
填写带时区的 ISO 8601 起止时间、号源上限、地点和参考费用。重新选择科室会清除已选医生。
医生创建后不能更换所属科室；排班的医生、时间和费用创建后固定，后续只能调整号源上限及
开放状态，上限不能低于已有预约数。参考费用按人民币分填写，`2500` 表示 25 元；模板不
收款，也不验证付款。

顾客先在 **我的就诊人** 填写姓名，联系方式和关系均可空。模板不要求身份证号、症状、诊断
或病历，不核验自然人身份及亲属关系。在 **排班挂号** 选中排班，核对医生、科室、时间、
地点及费用，再选择启用的就诊人预约。服务器在事务中检查归属、记录启用状态、时间、重复
预约和号源余量。目录显示开放不代表提交时一定仍有余号。

顾客可在排班开始前取消已预约记录。**恢复本人预约** 复用同一预约、同一就诊人和同一排班，
重新核验可约条件，并更新就诊人及排班快照，包括参考费用；不要为同一就诊人、同一排班
再次提交新预约。更换时段时，按规则取消后另约其他排班。院方可取消仍处于已预约状态的记录，
包括已经开始的排班。关闭排班不会自动取消预约，需要逐条处理并另行联系就诊人。

工作人员在排班时段内登记签到，再将已签到预约标记为处理完成。这些是行政状态，不生成
临床记录，也不证明已经诊疗。操作说明只填写行政原因。**刷新列表** 用于手动获取最新记录；
首版不包含实时排队、叫号屏、通知、支付退款、实名核验、病历、处方或 HIS 对接，相关外部
服务在导出后接入。完整流程以 React／Vue 网页导出为目标；原生移动端、小程序运行时需要
另行开发和设备测试。模块组合继续受既有后端大小预算限制。

## 餐厅点餐模板

后端库中的 **餐厅点餐** 模板（`food-ordering`）生成单店使用的 React／Vue 页面与 NestJS
后端。保持 **NestJS Backend Provider** 启用，打开 **浏览后端库**，选择模板并核对准备要求，
再点击 **使用模板**。无需额外安装画布模块插件。创建会一次增加 8 个页面，支持一步撤销；
新文档原默认页保留，因此共有 9 页。

| 页面         | 路由               | 用途                                         |
| ------------ | ------------------ | -------------------------------------------- |
| 业务系统登录 | `/business-login`  | 通过 OIDC 身份服务登录。                     |
| 账号设置     | `/account-setup`   | 登记本人资料。                               |
| 点餐菜单     | `/menu`            | 浏览可点菜品，保存希望购买的总数量。         |
| 点餐购物车   | `/cart`            | 核对本人有效购物车明细、修改数量或移除菜品。 |
| 确认下单     | `/checkout`        | 选择购物车汇总，提交堂食或到店自取订单。     |
| 我的点餐订单 | `/food-orders`     | 查看本人订单、明细及记录，取消待接单订单。   |
| 厨房订单     | `/kitchen`         | 接单、开始制作、标记出餐、完成或拒绝订单。   |
| 菜单管理     | `/menu-management` | 新增及编辑菜品、价格和可点状态。             |

按照导出 README 配置 PostgreSQL 与 OIDC。普通登录用户可以使用本人的购物车和订单；
身份管理员为餐厅工作人员授予 `food-manager` 后，才可管理菜单和厨房订单。登记资料不会
自动授予角色。工作人员新增菜品时需选择本人已登记资料。价格和金额按最小货币单位填写
整数；本模板使用 CNY，`1200` 表示 12 元。下架／售罄只是菜单可点状态，不维护食材或
菜品库存数量。图片字段仅保存 URL；这些表单不提供图片上传或自动图片展示。

保存数量会**替换**该菜品在购物车中的总数量，范围为 1–99。确认下单须填写联系人，堂食
还须填写桌号；电话和备注可空。服务器在事务中核验购物车版本、当前价格及可点状态，
并创建订单明细快照和操作记录；客户端不提交自算总额。发生冲突后请重新核对当前购物车
再提交。菜品涨跌价后，需重新保存所需数量以接受现价，再刷新核对购物车。
已移除或已结算的购物车明细不会作为可编辑历史列表显示。

厨房按待接单 → 已接单 → 制作中 → 已出餐 → 已完成逐步处理。顾客只能取消待接单订单；
工作人员可拒绝尚未完成的订单。**刷新列表**会按当前搜索和筛选重新加载首页并清除选择，
保留尚未提交的表单内容和已保存／进行中的请求。厨房目前需要手动刷新，不包含实时推送、
轮询、支付、退款、配送、短信或打印机接入；这些服务在导出后开发。模板不验证桌号归属，
也不会部署服务。完整流程以 React／Vue 网页导出为目标，原生移动端和小程序运行时需要
另行开发与设备测试。更多准备要求见[业务模板说明](/development/backend-business-templates)。

## 视频网站与直播频道模板

后端库中的 **视频网站与直播频道** 模板（`video-live`）生成可编辑的 React 或 Vue 页面，
以及管理视频、直播频道记录和私人收藏的 NestJS 后端。

1. 在 **设置 → 插件 → 浏览** 中安装 **Video**，切换到 **已安装** 将其启用，再关闭设置。
   同时保持 **NestJS Backend Provider** 启用。
2. 打开 **浏览后端库**，选择 **视频网站与直播频道**，查看角色及准备要求。可使用新应用文档，
   或把它作为业务模块加入兼容的已有应用。
3. 点击 **使用模板**。新应用会增加下表中的 7 个页面；原默认页保留，因此新文档共有 8 页。
   整次创建支持一步撤销。追加模块时会共享已有的登录页和账号设置页，不会重复创建。
4. 配置 PostgreSQL 与 OIDC，按照导出 README 运行前后端。登录后先在 **账号设置** 登记本人资料。
   身份服务管理员须为创作者授予 `media-creator`，或为管理所有创作者内容的运营人员授予
   `media-admin`。创建模板和登记资料都不会自动授予角色。

| 页面           | 路由                | 用途                                                             |
| -------------- | ------------------- | ---------------------------------------------------------------- |
| 业务系统登录   | `/business-login`   | 通过配置的 OIDC 身份服务登录。                                   |
| 账号设置       | `/account-setup`    | 登记当前登录用户的资料。                                         |
| 视频目录       | `/videos`           | 公开浏览已发布视频，选中后观看；登录后可收藏。                   |
| 直播频道       | `/live`             | 公开浏览已排期、已标记开播或结束的频道。                         |
| 我的收藏       | `/favorites`        | 仅查看、取消或恢复自己的视频收藏。                               |
| 创作者视频管理 | `/creator/videos`   | 创建草稿、编辑、发布、归档、恢复视频，并查看操作记录。           |
| 创作者频道管理 | `/creator/channels` | 编辑频道、设置排期、标记开播或结束、归档、恢复，并查看操作记录。 |

在 **创作者视频管理** 中选择本人已登记资料、创建草稿，并在发布前填写公开播放地址。
MP4、WebM 使用浏览器的视频能力，具体编码也须受该浏览器支持。路径以 `.m3u8` 结尾的 HLS
地址使用随导出打包的 `hls.js` 播放器；浏览器媒体能力不兼容时会明确提示不支持，不会静默
退回原生 HLS。播放列表及引用的媒体资源均须使用公开 HTTPS 地址并允许 CORS；受控 HLS
加载器不接受重定向，也不会携带凭据。

在视频或频道列表中选中记录后，点击 **加载视频**。切换记录会清除旧播放器，并要求再次点击
加载。可同时填写封面地址；这些字段不得包含秘密、签名令牌或私有地址。收藏只保留标题与视频
引用，不保存播放地址副本；恢复收藏时会重新检查视频当前仍已发布，观看时请返回公开目录读取
当前版本。

频道先创建草稿，再设置未来排期。**标记开播** 要求记录已排期并填写播放地址，可在预告时间前
手动操作；**标记结束** 再次更新记录。这些标记不验证直播流是否可用，不会启停编码器，也不会
生成回放。真实推流、上传、转码、媒体存储、CDN、付费访问及流状态监控均需在导出后接入；
模板不打包视频文件，也不会部署这些服务。

本模板以 React／Vue 网页导出为目标。完整播放器与后端流程尚未在 Expo、Flutter 或小程序
导出中实现或验证，手机设备也需单独测试。后端准备与模块组合方式见[业务模板说明](/development/backend-business-templates)。

## VR 看房与租房模板

### 导出到 RN、Flutter 和 Taro

配置好房间与热点后，启用对应导出插件，使用 **文件 → 导出 → Expo／Flutter／Taro 工程**。
如果需要 ZIP 自带示例图，请先在 VR 属性面板点击 **使用真实住宅示例**。解压后的 README
包含平台要求与完整命令；编辑器只导出源码，不安装依赖或调用移动端构建工具。

| 目标               | 在解压目录执行的准备步骤                                                                                       | 导出结果                                                                                                                   |
| ------------------ | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| Expo／React Native | `npm install`，然后 `npm start`                                                                                | 启动前自动准备播放器，全景区域使用 WebView，外围页面仍为原生组件。直接运行 Expo 命令时需先执行 `npm run prepare:vr-tour`。 |
| Flutter            | `npm install --prefix vr-tour-web`、`node vr-tour-web/build.mjs`，再按 README 执行 Flutter 命令                | WebView 加载随应用打包的本地 HTML。                                                                                        |
| Taro／微信小程序   | `npm install --prefix vr-tour-web`、`node vr-tour-web/build.mjs`，自行把 `vr-tour-web/dist/` 部署为 HTTPS 网页 | 在导出的配置文件填写 `VR_TOUR_BASE_URL`，并在微信后台配置业务域名；点击看房按钮后进入独立全屏页面。                        |

**个人类型微信小程序不支持 web-view。** 导出不会自动部署网站。三端沿用文档语言、加载按钮
和房间热点操作。源码包保留 8K 原图，移动端显示图限制为最大 4096 像素宽以减少 GPU 占用，
但首次解码仍可能占用较多内存。

当前支持已配置的静态房间。动态房源 `panoramaUrl` 绑定会明确提示不支持，不会误显示默认房间；
实时租房后端绑定请使用 React／Vue 导出。手机真机检查、签名和部署需在导出后完成；陀螺仪和
头显模式尚未接入。

### 安装与配置看房组件

安装 **OpenPencil VR 看房** 时会一起下载两张 8K 真实住宅示例，合计约 6.5 MB。
首次安装需要联网，图片校验并完整缓存到本机后才完成安装；后续试用可直接读取缓存。
已安装旧版插件的用户可点击下方的示例按钮补齐资源，不需要卸载重装。

插入组件后，在属性面板点击 **使用真实住宅示例**，然后打开桌面预览并点击 **加载全景**。
可以拖动、缩放，或点击热点切换示例。应用示例会替换当前场景，并支持一步撤销；两张图来自
不同住宅，热点不代表真实相邻房间。图片由 Greg Zaal / Poly Haven 提供，采用 CC0 许可。

中文编辑器新插入的组件默认使用中文，可通过 **界面语言** 选择简体中文或 English。
该设置控制预览和导出页面的按钮、提示、错误及无障碍说明；已有文档保持原来的语言设置。
示例图片随文档保存，复制组件到另一文档也会携带引用图片。导出仅附带实际引用的示例图，
不需要手动复制图片；React／Vue 的来源说明位于 `public/assets/vr-tour/SOURCES.json`，
Expo／Flutter／Taro 则位于 `vr-tour-web/SOURCES.json`。卸载插件不会删除文档中的图。

在 **设置 → 插件 → 浏览** 中安装并启用 **OpenPencil VR 看房**，再从画布工具栏的模块菜单
插入 **VR 看房**。属性面板可直接编辑房间名称、全景图片和跳转热点，选择初始房间与视角，
并设置热点的水平、垂直角度。删除房间会在同一个撤销步骤中移除指向它的热点，并修正初始房间。

请准备自己的 2:1 等距柱状全景图，支持 JPG、PNG、WebP，宽度为 512–8192 像素，每张不超过
24 MiB。公开 HTTPS 图片需允许 CORS；不支持带凭据、查询参数或片段的 URL。导出项目也可
把图片放入 `public/assets/vr-tour/`，使用 `/assets/vr-tour/<文件名>.jpg` 路径；仅填写路径不会
自动导入本机文件。

画布显示离线房间示意。桌面 React/Vue 预览及导出的 Web 页面支持全景拖动、缩放、键盘操作
与房间热点。点击加载后才请求全景图片；切换绑定房源会清除旧视图并要求重新加载。空图片
仍明确显示为示意图。普通浏览器 Compiler Preview 暂不支持此模块，本版也不提供头显 WebXR
或 GLB 模型内自由行走。

后端库新增 **租房看房** 模板，选中房源后会将其全景字段传给 VR 组件。创建模板或追加业务模块前
需启用 VR 看房插件。房东、租客角色及预约流程请参阅[业务模板说明](/development/backend-business-templates)。
