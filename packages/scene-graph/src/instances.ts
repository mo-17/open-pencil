...
function copyUnprotectedInstanceFields(
  compChild: SceneNode,
  instChild: SceneNode,
  instanceId: string,
  overrides: InstanceOverrideState,
...
function syncChildren(
  ...
  for (const [componentId, instance] of instChildMap) idMap.set(componentId, instance.id)
  ...
  for (const compChildId of compParent.childIds) {
    ...
    copyUnprotectedInstanceFields(compChild, instChild, instanceId, overrides, legacyOverrides)
  ...
  // Keep unmatched children stable, with legacy override compatibility.
