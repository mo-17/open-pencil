...
function pinnedWebhookURL(
  value: unknown,
  path: string,
  context: BackendValidationContext
): string | undefined {
  const text = boundedText(value, path, context, 2_048)
  ...
  try {
    ...
    context.diagnostics.push({
      code: 'backend-webhook-destination-url-invalid',
      severity: 'error',
      path,
      message: 'Webhook destinations must use a pinned public HTTPS URL.'
    })
    return undefined
  }
  ...
  if (
    ...
    context.diagnostics.push({
      ...
      path,
      message:
        'Webhook destinations must be credential-free public HTTPS URLs without query, fragment, or custom port.'
    })
    return undefined
  }
...
function webhookDestination(
  ...
  const name = boundedText(source.name, `${path}.name`, context, 128)
  const url = pinnedWebhookURL(source.url, `${path}.url`, context)
...
function validCronSegment(segment: string, minimum: number, maximum: number): boolean {
  ...
  if (!base || extra.length > 0) return false
  if (step !== undefined && (!validCronInteger(step, 1, maximum - minimum + 1) || base === '')) {
    return false
  }
  if (base === '*') return true
    ...
    secrets
      .filter(
        (secret): secret is Extract<BackendSecretRef, { kind: 'credential' }> =>
          secret.kind === 'credential' && secret.exposure === 'server' && secret.required === true
      )
  ...
  if (queues && webhookDestinations && automations) {
    ...
    if (new Set(credentialUses).size !== credentialUses.length) {
      context.diagnostics.push({
        ...
        message:
          'Each inbound endpoint and outbound destination requires a dedicated HMAC credential.'
      })
    }
    for (const automation of automations) {
