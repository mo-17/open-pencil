...
export interface BackendWebhookDestinationIR {
  id: string
  name: string
  /** Host resolves the complete HTTPS endpoint from this opaque, server-scoped credential. */
  endpointCredentialRef: BackendCredentialRef
  method: 'POST'
  contentType: 'application/json'
  /** Host deterministically serializes JSON, checks this byte limit, then signs and sends it. */
  maxPayloadBytes: number
  /** Release evidence must prove a distinct server-scoped credential with webhook-hmac purpose. */
