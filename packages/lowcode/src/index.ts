 ...
 * Lives in `@open-pencil/lowcode` so core, compiler, app, and external
 * consumers share one framework-agnostic implementation. Compiler + editor
 * previously held copies in `packages/compiler/src/ir/`; this package is now
 * the canonical owner while core retains compatibility exports for its
 * established lowcode-validation subpaths.
 */
export {
  type BinaryOp,
  type ExprAst,
