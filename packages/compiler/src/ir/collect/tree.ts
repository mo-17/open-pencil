...
function imageAssetPath(hash: string, bytes: Uint8Array): string {
  const normalized = hash.replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 48)
  const safeHash = normalized || `bytes-${bytes.length.toString(36)}`
  const suffix = normalized === hash ? '' : `-${stableAssetHashSuffix(hash)}`
  return `src/assets/openpencil-image-${safeHash}${suffix}.${imageExtension(bytes)}`
}

function stableAssetHashSuffix(value: string): string {
