...
export function emitSupabaseBackfillReviewSQLV2(application: BackendApplicationSpecV2): string {
  ...
  const lines = [
    ...
    '',
    '-- Statement 2 reads bounded high water and estimates both eligible and cursor-range batches.',
    'WITH "row_summary" AS (',
    '  SELECT',
    `    MAX(${cursorName}) FILTER (WHERE ${cursorName} BETWEEN 0 AND ${SUPABASE_BACKFILL_MAX_SAFE_CURSOR_V2}) AS "captured_high_water",`,
