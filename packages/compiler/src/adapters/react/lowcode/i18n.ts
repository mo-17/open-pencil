...
/** Phase 3 §9 v11 — primary language subtags written right-to-left. Matched
 *  against a locale code's primary subtag (`ar-EG` → `ar`), so region/script
 *  variants are covered. (`iw`/`ji` are legacy codes for `he`/`yi`.) */
...
const RTL_LOCALES = new Set([${[...RTL_LOCALES].map((c) => JSON.stringify(c)).join(', ')}])

/** Does this locale lay out right-to-left? (compares the primary subtag) */
...
`
    : ''
  const dirEffect = rtl
    ? `  // Reflect the active locale's writing direction on the document root so
  // CSS / layout flips between LTR and RTL languages.
  useEffect(() => {
