...
import { FLUTTER_VR_WEBVIEW_VERSION } from './vr-tour'
...
environment:
  sdk: ">=${hasVRTours ? '3.10.0' : '3.3.0'} <4.0.0"${hasVRTours ? '\n  flutter: ">=3.38.0"' : ''}

dependencies:
  flutter:
    sdk: flutter${hasVRTours ? `\n  webview_flutter: ${FLUTTER_VR_WEBVIEW_VERSION}` : ''}
...
export function buildFlutterReadme(
  options: CompilerOptions,
  router: boolean,
  hasVRTours = false
): string {
...
Use ${hasVRTours ? 'Flutter >=3.38.0 with Dart >=3.10.0. First complete the VR preparation commands below.' : 'a Flutter SDK compatible with Dart 3.3 or newer.'} From this directory, generate the platform directories, then resolve packages:
...
${hasVRTours ? 'The application uses Flutter Widgets, with the explicit VR WebView exception described below.' : 'The export uses native Flutter Widgets, not a WebView.'} It supports basic layout, text, raster images, buttons, text inputs, local/document state, local lists, and a safe subset of actions.${
    router
...
Unsupported web, ${hasVRTours ? 'other plugin' : 'plugin'}, remote-data, motion, upload, and advanced-form behavior is omitted and listed in the compiler warnings. OpenPencil did not run CocoaPods, Gradle, signing, a simulator, or a physical-device build.
`
}
...
export function buildFlutterWidgetTest(options: CompilerOptions, hasVRTours = false): string {
  const packageName = dartPackageName(options.packageName)
  return `import 'package:flutter_test/flutter_test.dart';
${hasVRTours ? "import 'package:flutter/foundation.dart';\n" : ''}
