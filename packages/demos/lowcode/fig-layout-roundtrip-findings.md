...
## 候选 5 — auto-layout 子节点 `derivedLayout.x/y` 的 `?? ` vs `0` nullish bug【high】

- **存**: `serialize.ts:373-412` `computeExportTransform` 对 auto-layout 子(父 H/V 且非 ABSOLUTE)**故意把 m02=0,m12=0**(Figma 兼容:位置由布局引擎算)。w/h 存活在 `nc.size`。`derivedLayout` 本身不序列化。
- **读**: `convert.ts:332-352` `visibleContainerDerivedLayout` 用 `{x: nc.transform?.m02 ?? 0, y: nc.transform?.m12 ?? 0, ...}` 重建 derivedLayout → 因存的是 (0,0),恒得 `{x:0,y:0,...}`。
...
| 5   | derivedLayout 0-nullish            | high | 小       | auto-layout 子钉 0,影响面大                   |
