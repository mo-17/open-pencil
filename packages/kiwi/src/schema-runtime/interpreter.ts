...
import { assertSchemaBudgets, validateDynamicSchema } from './validate'
...
export interface CompileSchemaOptions {
  ...
  /** Validate untrusted binary schema names and identifiers before compilation. */
...
function interpretDecode(
  ...
  limits?: Readonly<KiwiRuntimeLimits>,
  limitedDecodeBuffers?: WeakSet<ByteBuffer>
) {
  ...
  return function (bb: ByteBuffer | Uint8Array): RuntimeMessage {
    const sourceBuffer = bb instanceof ByteBuffer ? bb : null
    let buffer: ByteBuffer
    if (
      bb instanceof self.ByteBuffer &&
      (!limitedDecodeBuffers || limitedDecodeBuffers.has(bb))
    ) {
      buffer = bb
    } else {
      const data = sourceBuffer ? sourceBuffer.toUint8Array() : (bb as Uint8Array)
      buffer = new self.ByteBuffer(data, limits)
      limitedDecodeBuffers?.add(buffer)
    }
    if (sourceBuffer && sourceBuffer !== buffer) buffer.offset = sourceBuffer.offset
    ...
    try {
      ...
      buffer.leaveDecode()
      if (sourceBuffer && sourceBuffer !== buffer) sourceBuffer.offset = buffer.offset
    }
  }
}
...
export function compileSchemaRuntime(
  schema: Schema,
  options: CompileSchemaOptions = {}
): RuntimeCodec {
  const limits = normalizeKiwiRuntimeLimits(options.limits)
  if (options.validateDynamicSchema) validateDynamicSchema(schema, limits)
  else assertSchemaBudgets(schema, limits)
  ...
  const RuntimeByteBuffer: typeof ByteBuffer = limits
    ...
    : ByteBuffer
  const limitedDecodeBuffers = limits ? new WeakSet<ByteBuffer>() : undefined
  let result: RuntimeCodec = { ByteBuffer: RuntimeByteBuffer }
