...
import { parseBackendClientActionFields } from '@open-pencil/lowcode/backend'
import type {
  ActionDef,
  ActionKind,
  AnalyticsConfig,
...
function validateActionAt(
  ...
  if (
    ...
    return validateBackendClientActionAt(where, value)
  const fieldsR = validatePerKindFields(where, kind as ActionKind, value)
  if (!fieldsR.ok) return fieldsR
  return { ok: true, action: buildActionFromValidated(value.id, kind as FlatActionKind, value) }
}
...
function validateBackendClientActionAt(
  where: string,
  value: Record<string, unknown>
): { ok: true; action: ActionDef } | { ok: false; error: string } {
  const parsed = parseBackendClientActionFields(value)
  if (!parsed.ok) return failAt(where, parsed.diagnostics.map((entry) => entry.message).join(' '))
  if (parsed.value.kind === 'backendAuth') return { ok: true, action: parsed.value }
  const branches = validateResultBranches(where, value)
  if (!branches.ok) return branches
  const onSuccess = builtBranch(value, 'onSuccess')
  const onError = builtBranch(value, 'onError')
  return {
    ok: true,
    action: {
      ...parsed.value,
...
type FlatActionKind = Exclude<
  ActionKind,
  'backendAuth' | 'backendRequest' | 'backendCommand' | 'backendCommandRecovery'
>
