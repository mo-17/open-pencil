...
import { isAtomicTool, type ToolDef, type ToolCtx } from '#core/tools/schema'
...
export function executeAtomicTool(
  ...
  options: { signal?: AbortSignal; isLive?: () => boolean; label?: string; context?: ToolCtx } = {}
): unknown {
  ...
  try {
    const result = def.execute(figma, args, options.context)
