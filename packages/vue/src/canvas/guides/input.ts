...
type GuideDeleteEvent = Pick<KeyboardEvent, 'code' | 'composedPath' | 'preventDefault' | 'target'>
...
function comesFromEditableTarget(event: GuideDeleteEvent): boolean {
  const path = typeof event.composedPath === 'function' ? event.composedPath() : [event.target]
  return path.some((target): target is EventTarget => target !== null && isEditableTarget(target))
}
...
export function createGuideInput({
  ...
  function deleteSelected(event: GuideDeleteEvent): boolean {
    if (event.code !== 'Delete' && event.code !== 'Backspace') return false
    if (comesFromEditableTarget(event)) return false
