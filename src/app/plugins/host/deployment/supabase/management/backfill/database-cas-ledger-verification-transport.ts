...
function requestDeadline(caller: AbortSignal | undefined, timeoutMs: number): RequestDeadline {
  ...
  const timeout = setTimeout(() => {
    timedOut = true
    controller.abort(new DOMException('Database CAS ledger verification timed out', 'TimeoutError'))
  }, timeoutMs)
  return {
...
function abortReason(): Error {
  return new DOMException('Supabase database CAS ledger verification aborted', 'AbortError')
}
