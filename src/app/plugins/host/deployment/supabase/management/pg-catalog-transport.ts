...
function aggregateSQL(queries: readonly FixedQuery[]): string {
  ...
  const resultPairs = queries.flatMap((query) => {
    ...
    return [
      `'${query.queryId}'`,
      `COALESCE((SELECT pg_catalog.jsonb_agg(pg_catalog.to_jsonb(row_data)) FROM "${name}" AS row_data), '[]'::pg_catalog.jsonb)`
    ]
  })
...
function sharedAggregateSQL(): string {
  if (SUPABASE_PG_CATALOG_AGGREGATE_SQL_SOURCE.endsWith('\r\n')) {
    return SUPABASE_PG_CATALOG_AGGREGATE_SQL_SOURCE.slice(0, -2)
  }
  return SUPABASE_PG_CATALOG_AGGREGATE_SQL_SOURCE.endsWith('\n')
    ? SUPABASE_PG_CATALOG_AGGREGATE_SQL_SOURCE.slice(0, -1)
    : SUPABASE_PG_CATALOG_AGGREGATE_SQL_SOURCE
}

/** One immutable SQL statement shared verbatim with the native desktop authority. */
export const SUPABASE_PG_CATALOG_AGGREGATE_SQL = sharedAggregateSQL()

if (SUPABASE_PG_CATALOG_AGGREGATE_SQL !== aggregateSQL(SUPABASE_PG_CATALOG_FIXED_QUERIES)) {
  throw new TypeError('The shared Supabase pg_catalog query is out of sync')
}

export const SUPABASE_PG_CATALOG_AGGREGATE_PARAMETERS = Object.freeze(
...
/** Shared strict decoder for fixed native and fetch-based pg_catalog transports. */
export function parseSupabasePgCatalogAggregateResponse(
  value: unknown,
  request: SupabasePgCatalogReadRequest,
  authority: SupabasePgCatalogProjectAuthority
): SupabasePgCatalogReadResult {
  validateReadRequest(request, authority)
...
export function createSupabaseManagementPgCatalogTransport(
  ...
  return Object.freeze({
    ...
    async runReadOnlyCatalogQueries(
      ...
      if (authority === null) fail('invalid-request')
      validateReadRequest(request, authority)
      ...
      return parseSupabasePgCatalogAggregateResponse(value, request, authority)
    }
  })
}
