...
export interface SupabaseManagementStorageAuthority {
  readonly projectRef: string
  readonly accountId: string
  readonly grantGeneration: string
}

export interface SupabaseStorageTestUser {
  /** Expected Auth user id. The live /auth/v1/user response must match it. */
  readonly userId: string
  /** Operation-scoped JWT resolved from the credential vault. */
