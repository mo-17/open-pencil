...
import type { DesktopSupabaseBackendReviewResult } from './desktop-supabase-backend-review'
import type { SupabaseEdgeFunctionReleaseReceipt } from './supabase/edge-function-release'
import type { SupabaseStagingVerificationResult } from './supabase/staging-verifier'
import type { SupabaseStorageIsolationReceipt } from './supabase/storage-isolation-verifier'
...
export interface DesktopSupabaseStagingTestUserInput {
  readonly userId: string
  /** Operation-scoped JWT. It must never be persisted or returned. */
...
export interface DesktopSupabaseStrictStagingVerificationInput {
  ...
  readonly readPersonalAccessToken: string
  ...
  return Object.freeze({
    async verify(
      ...
      try {
        ...
        readPersonalAccessToken = await dependencies.resolveReadCredential()
        ...
        if (readPersonalAccessToken === writePersonalAccessToken) {
