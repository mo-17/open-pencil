...
import { verifySupabaseStagingRelease } from '@/app/plugins/host/deployment/supabase/staging-verifier'
import {
...
export interface AppDesktopSupabaseBackendStagingVerificationForTestingOptionsV1 {
...
function failNativeVerification(cause: SupabaseManagementNativeError): never {
  if (cause.code === 'write-credential-missing') {
    throw new DesktopSupabaseBackendStagingVerificationError('write-credential-missing')
  }
  if (cause.code === 'credential-not-independent') {
    throw new DesktopSupabaseBackendStagingVerificationError('write-credential-not-independent')
  }
  if (cause.code === 'credential-missing' || cause.code === 'credential-unavailable') {
    throw new DesktopSupabaseBackendStagingVerificationError('credential-missing')
  }
  if (cause.code === 'credential-changed') {
    throw new DesktopSupabaseBackendStagingVerificationError('grant-changed')
  }
...
async function liveSnapshot(
  ...
  if (
    (runtime.readFetcher && input.readPersonalAccessToken === null) ||
    (!runtime.readFetcher && input.readPersonalAccessToken !== null)
  ) {
    throw new DesktopSupabaseBackendStagingVerificationError('verification-failed')
  }
  const transport = runtime.readFetcher
    ? createSupabaseManagementPgCatalogTransport({
  ...
  if (
    !runtime.readFetcher &&
    !isProductionNativeSupabaseManagementPgCatalogTransportV1(transport)
  ) {
    throw new DesktopSupabaseBackendStagingVerificationError('verification-failed')
  }
  let authority
  try {
    authority = await transport.getProjectAuthority({
      projectRef: input.projectRef,
      grantGeneration: input.grantGeneration
    })
  } catch (cause) {
    if (!runtime.readFetcher && cause instanceof SupabaseManagementNativeError) {
      return failNativeVerification(cause)
    }
    throw cause
  }
...
function createAppDesktopSupabaseBackendStagingVerificationServiceWithOptions(
  options: AppDesktopSupabaseBackendStagingVerificationForTestingOptionsV1
) {
...
/** Production owner: catalog inspection cannot be replaced with a renderer fetcher. */
export function createAppDesktopSupabaseBackendStagingVerificationService() {
  return createAppDesktopSupabaseBackendStagingVerificationServiceWithOptions({})
}

/** Explicit testing composition; it never receives production native transport provenance. */
export function createAppDesktopSupabaseBackendStagingVerificationServiceForTestingV1(
  options: AppDesktopSupabaseBackendStagingVerificationForTestingOptionsV1
) {
  if (typeof options.readFetcher !== 'function') {
    throw new TypeError('Testing Supabase staging verification requires an explicit readFetcher.')
  }
  return createAppDesktopSupabaseBackendStagingVerificationServiceWithOptions(options)
}
