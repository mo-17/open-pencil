...
import type {
  Fill,
  FillType,
  FigmaRawStructuralFieldPresence,
  GeometryPath,
  SceneNode,
  SourceMetadata
...
const FIGMA_RAW_STRUCTURAL_FIELDS = [
  'name',
  'visible',
  'opacity',
  'size',
...
export type DecodedYjsNodeProps = Partial<SceneNode> & Record<string, unknown>
...
export function decodeNodeFromYjs(ynode: YjsNodeLike): DecodedYjsNodeProps {
  ...
  return props as DecodedYjsNodeProps
}
...
export function normalizeSourceMetadata(source: unknown): SourceMetadata {
  ...
  return {
    ...
    fig: {
      rawNodeType: stringOrNull(fig.rawNodeType),
      rawStructuralFieldPresence: normalizeRawStructuralFieldPresence(
        fig.rawStructuralFieldPresence
      ),
...
function normalizeRawStructuralFieldPresence(
  value: unknown
): FigmaRawStructuralFieldPresence | null {
  if (!isRecord(value)) return null
  if (!FIGMA_RAW_STRUCTURAL_FIELDS.every((field) => typeof value[field] === 'boolean')) return null
  return Object.fromEntries(
    FIGMA_RAW_STRUCTURAL_FIELDS.map((field) => [field, value[field] as boolean])
  ) as FigmaRawStructuralFieldPresence
}
