...
const { items, currentProviderKey, templateProviderKey, blockReason, busy, loading, error } =
  defineProps<{
...
const selectedProvider = computed(() =>
  selected.value?.kind === 'provider'
    ? selected.value
    : items.find((item) => item.kind === 'provider' && item.descriptorKey === templateProviderKey)
)
...
const canApply = computed(() => {
  ...
  return !blockReason && !stale.value && validAuth.value
})
...
watch(open, (value) => {
  if (value) reviewedProviderKey.value = templateProviderKey
})
function apply(): void {
  const item = selected.value
  ...
  } else emit('useTemplate', item.id, authentication.value, reviewedProviderKey.value)
}
        ...
        >
          <div class="flex w-full items-start gap-3">
            <span :class="styles.icon()">
              ...
              <icon-lucide-shopping-bag v-else-if="item.id === 'single-sku-shop'" class="size-5" />
              <icon-lucide-notebook-pen v-else class="size-5" />
            </span>
    ...
    <AppDialogFooter>
      <div :class="styles.footer()">
        ...
        <AppButton
          ...
          @click="apply"
        >
          {{ selected?.kind === 'provider' ? copy.selectProvider : copy.useTemplate }}
        </AppButton>
      </div>
    </AppDialogFooter>
  </AppDialogRoot>
